...
Elm.Main.make = function (_elm) {
   ...
   var view = F2(function (address,model) {
      ...
      _U.list([A2($Html.button,_U.list([A2($Html$Events.onClick,address,Decrement)]),_U.list([$Html.text("-")]))
              ...
              ,A2($Html.h3,_U.list([]),_U.list([$Html.text("Increment Clicked "),$Html.text($Basics.toString(model.inc))]))
              ,A2($Html.h3,_U.list([]),_U.list([$Html.text("Decrement Clicked "),$Html.text($Basics.toString(model.dec))]))]));
   });
   var main = $StartApp$Simple.start({model: {count: 0,dec: 0,inc: 0},update: update,view: view});
   var Model = F3(function (a,b,c) {    return {count: a,dec: b,inc: c};});
   return _elm.Main.values = {_op: _op,Model: Model,Increment: Increment,Decrement: Decrement,update: update,view: view,main: main};
};
